...
describe("Account", () => {
    ...
    describe("constructors", () => {
        ...
        it("creates a new from seed", () => {
            ...
            expect(account.privateKey().to_string()).equal(
                beaconPrivateKeyString,
            );
        ...
        it("throws an error if parameters are invalid", () => {
            expect(
                () => new Account({ privateKey: "invalidPrivateKey" }),
            ).throw();
        });
        ...
        it("can encrypt an account and decrypt to the same account", () => {
            ...
            for (const account of [accountFromString, accountFromObject]) {
                // Test that expected output is generated
                expect(account.privateKey().to_string()).equal(
                    newAccount.privateKey().to_string(),
                );
                expect(account.viewKey().to_string()).equal(
                    newAccount.viewKey().to_string(),
                );
                expect(account.address().to_string()).equal(
                    newAccount.toString(),
                );
    ...
    describe("isValidAddress", () => {
        ...
        it("returns false for invalid address bytes", () => {
            expect(Account.isValidAddress(new Uint8Array([1, 2, 3]))).equal(
                false,
            );
    ...
    describe("View Key Record Decryption", () => {
        it("decrypts a record in ciphertext form", () => {
            ...
            const decryptedRecord = account.decryptRecord(
                recordCiphertextString,
            );
        ...
        it("doesnt decrypt records from other accounts nor identifies them as the record owner", () => {
            ...
            expect(account.ownsRecordCiphertext(foreignCiphertextString)).equal(
                false,
            );
        ...
        it("decrypts an array of records in ciphertext form", () => {
            ...
            expect(decryptedRecords).deep.equal([
                recordPlaintextString,
                recordPlaintextString,
            ]);
        });
    ...
    describe("Sign and Verify", () => {
        it("verifies the signature on a message", () => {
            const account = new Account();
            const other_message = Uint8Array.from([
                104, 101, 108, 108, 111, 32, 120, 121, 114, 108, 99,
            ]);
            ...
            const isSigValidForWrongMessage = account.verify(
                other_message,
                signature,
            );
            expect(verify_spy.calledWith(other_message, signature)).equal(true);
            const isSigValidForMultipleMessages = account.verify(
                other_message,
                other_signature,
            );
            expect(verify_spy.calledWith(other_message, other_signature)).equal(
                true,
            );
    ...
    describe("Account Lifecycle & Zeroization", () => {
        it("destroy prevents further use of the account", () => {
            ...
            expect(() =>
                account.verify(message, new Account().sign(message)),
            ).to.throw(/destroyed/);
            expect(() => account.clone()).to.throw(/destroyed/);
            expect(() => account.toString()).to.throw(/destroyed/);
            expect(() => account.encryptAccount("password")).to.throw(
                /destroyed/,
            );
            expect(() => account.decryptRecord("ciphertext")).to.throw(
                /destroyed/,
            );
            expect(() => account.decryptRecords(["ciphertext"])).to.throw(
                /destroyed/,
            );
        });
        ...
        it("clone produces a working independent copy", () => {
            ...
            expect(cloned.privateKey().to_string()).to.equal(
                account.privateKey().to_string(),
            );
            expect(cloned.viewKey().to_string()).to.equal(
                account.viewKey().to_string(),
            );
            expect(cloned.address().to_string()).to.equal(
        ...
        it("fromCiphertext produces a working account", () => {
            ...
            const recovered = Account.fromCiphertext(
                ciphertext,
                "testpassword",
            );

            expect(recovered.privateKey().to_string()).to.equal(
                account.privateKey().to_string(),
            );
            expect(recovered.viewKey().to_string()).to.equal(
                account.viewKey().to_string(),
            );
            expect(recovered.address().to_string()).to.equal(
                account.address().to_string(),
            );
        ...
        it("accepts a PrivateKey object in AccountParam", () => {
            ...
            expect(fromPk.privateKey().to_string()).to.equal(
                beaconPrivateKeyString,
            );
