...
describe("Account", () => {
    describe("constructors", () => {
        ...
        test("creates a new from seed", () => {
            ...
            expect(account.privateKey().to_string()).toEqual(
                beaconPrivateKeyString,
            );
        ...
        test("can encrypt an account and decrypt to the same account", () => {
            ...
            for (const account of [accountFromString, accountFromObject]) {
                ...
                expect(account.viewKey().to_string()).toEqual(
                    newAccount.viewKey().to_string(),
                );
                expect(account.address().to_string()).toEqual(
                    newAccount.toString(),
                );
    ...
    describe("View Key Record Decryption", () => {
        ...
        test("doesnt decrypt records from other accounts nor identifies them as the record owner", () => {
            ...
            expect(account.ownsRecordCiphertext(foreignCiphertextString)).toBe(
                false,
            );
        ...
        test("decrypts an array of records in ciphertext form", () => {
            ...
            expect(decryptedRecords).toEqual([
                recordPlaintextString,
                recordPlaintextString,
            ]);
        });
