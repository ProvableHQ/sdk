...
describe("merkle_tree lib, buildTree", () => {
    ...
    it("should throw error for empty leaves", async () => {
        expect(() => sealance.buildTree([])).to.throw(
            `Leaves array cannot be empty`,
        );
    });

    it("should throw error for odd number of leaves", async () => {
        expect(() =>
            sealance.buildTree(["1field", "2field", "3field"]),
        ).to.throw(`Leaves array must have even number of elements`);
    });
});
...
describe("merkle_tree lib, generateLeaves", () => {
    ...
    it("should throw error when exceeding max tree capacity", () => {
        ...
        expect(() =>
            sealance.generateLeaves(tooManyAddresses, maxDepth),
        ).to.throw("Leaves limit exceeded");
    });
});
...
describe("merkle_tree lib, getSiblingPath", () => {
    it("should handle odd leaf index (test sibling calculation branch)", () => {
        ...
        expect(formattedProof).to.equal(
            "[{siblings: [2field, 1field, 4560646903308595113151029585344265575242682454899063992850623350939538444867field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field], leaf_index: 1u32}, {siblings: [2field, 1field, 4560646903308595113151029585344265575242682454899063992850623350939538444867field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field], leaf_index: 1u32}]",
        );
    });
});
...
describe("merkle_tree lib, formatMerkleProof", () => {
    it("should format merkle proof correctly", () => {
        ...
        const indices = sealance.getLeafIndices(
            tree,
            "aleo1rhgdu77hgyqd3xjj8ucu3jj9r2krwz6mnzyd80gncr5fxcwlh5rsvzp9px",
        );
        ...
        expect(formattedProof).to.equal(
            "[{siblings: [0field, 3501665755452795161867664882580888971213780722176652848275908626939553697821field, 7162588494104628985065247846816902886874444240334527780589295712502521092900field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field], leaf_index: 0u32}, {siblings: [3501665755452795161867664882580888971213780722176652848275908626939553697821field, 0field, 7162588494104628985065247846816902886874444240334527780589295712502521092900field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field, 0field], leaf_index: 1u32}]",
        );
    });
});
