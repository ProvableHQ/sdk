import { buildTree, generateLeaves, getLeafIndices, getSiblingPath, ZERO_ADDRESS } from "../src/sealance-merketree-support.js";
...
describe("merkle_tree lib, buildTree", () => {
  ...
  it("should throw error for empty leaves", async () => {
    expect(() => buildTree([])).to.throw(`Leaves array cannot be empty`);
  });

  it("should throw error for odd number of leaves", async () => {
    expect(() => buildTree(["1field", "2field", "3field"])).to.throw(`Leaves array must have even number of elements`);
  });
});
...
describe("merkle_tree lib, generateLeaves", () => {
  ...
  it("should throw error when exceeding max tree capacity", () => {
    ...
    expect(() => generateLeaves(tooManyAddresses, maxDepth)).to.throw("Leaves limit exceeded");
    });
});
