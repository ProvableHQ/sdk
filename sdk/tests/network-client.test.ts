...
describe("NodeConnection", () => {
    ...
    describe("getBlock", () => {
        ...
        it("should throw an error if the request fails", async () => {
            await expect(connection.getBlock(99999999)).rejects.toThrow(
    ...
    describe("getBlockRange", () => {
        it("should return an array of Block objects", async () => {
            ...
            expect(((blockRange as Block[])[0] as Block).block_hash).toBe(
                "ab1hap8jlxaz66yt887gxlgxptkm2y0dy72x529mq6pg3ysy9tzwyqsphva9c",
            );
            expect(((blockRange as Block[])[1] as Block).block_hash).toBe(
                "ab18dzmjgqgk5z6x4gggezca7aenqts7289chvhus4a7ydrcj4apvrqq5j5h8",
            );
        }, 60000);
    ...
    describe("getDeploymentTransactionForProgram", () => {
        it("should return a Transaction object", async () => {
            const transaction =
                await connection.getDeploymentTransactionForProgram(
                    "hello_hello.aleo",
                );
    ...
    describe("getDeploymentTransactionIDForProgram", () => {
        it("should return a Transaction object", async () => {
            const transaction =
                await connection.getDeploymentTransactionIDForProgram(
                    "hello_hello.aleo",
                );
    ...
    describe("getProgram", () => {
        ...
        it("should throw an error if the request fails", async () => {
            ...
            await expect(connection.getProgram(program_id)).rejects.toThrow(
    ...
    describe("getTransaction", () => {
        it("should return a Transaction object", async () => {
            const transaction = await connection.getTransaction(
                "at1u833jaha7gtqk7vx0yczcg2njds2tj52lyg54c7zyylgfjvc4vpqn8gqqx",
            );
            expect((transaction as TransactionModel).type).toBe("deploy");
        }, 60000);
    });
    ...
    describe("getTransactions", () => {
        it("should return an array of Transaction objects", async () => {
            ...
            expect((transactions as TransactionModel[]).length).toBeGreaterThan(
                0,
            );
        }, 60000);

        it("should throw an error if the request fails", async () => {
            await expect(connection.getTransactions(999999999)).rejects.toThrow(
    ...
    describe("getProgramImports", () => {
        it("should return the correct program import names", async () => {
            const importNames =
                await connection.getProgramImportNames("aleoswap05.aleo");
    ...
    describe("Mappings", () => {
        it("should find program mappings and read mappings", async () => {
            ...
            const mappingValue = await connection.getProgramMappingValue(
                "credits.aleo",
                "account",
                "aleo1rlwt9w0fl242h40w454m68vttd6vm4lmetu5r57unm5g354y9yzsyexf0y",
            );
