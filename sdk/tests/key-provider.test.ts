    ...
    describe("getKeys", () => {
        ...
        it("Should use cache when set and not use it when not", async () => {
            ...
            const [provingKey, verifyingKey] = <FunctionKeyPair>(
                await keyProvider.transferKeys("private")
            );
        ...
        it("Should fetch verifying keys stored as text", async () => {
            ...
            const [provingKey, verifyingKey] = <FunctionKeyPair>(
                await provider.fetchKeys(
                    "https://pub-65a47b199b944d48a057ca6603a415a2.r2.dev/tree_mnist_2.prover.30e265c",
        ...
        it.skip("Should not fetch offline keys that haven't already been stored", async () => {
            ...
            const [setValidatorStateProver, setValidatorStateVerifier] = <
                FunctionKeyPair
            >await keyProvider.fetchKeys(
                CREDITS_PROGRAM_KEYS.set_validator_state.prover,
                CREDITS_PROGRAM_KEYS.set_validator_state.verifier,
            ...
            const [
                unbondDelegatorAsValidatorProver,
                ...
                await keyProvider.fetchKeys(
                    ...
                    CREDITS_PROGRAM_KEYS.unbond_delegator_as_validator.locator,
                )
            );
            ...
            offlineKeyProvider.insertUnbondDelegatorAsValidatorKeys(
                unbondDelegatorAsValidatorProver,
            );
            ...
            expect(bondPublicProver.checksum()).toEqual(
                bondPublicProverLocal.checksum(),
            );
            expect(bondPublicVerifier.checksum()).toEqual(
                bondPublicVerifierLocal.checksum(),
            );
            expect(claimUnbondPublicProver.checksum()).toEqual(
                claimUnbondPublicProverLocal.checksum(),
            );
            expect(claimUnbondVerifier.checksum()).toEqual(
                claimUnbondVerifierLocal.checksum(),
            );
            expect(feePrivateProver.checksum()).toEqual(
                feePrivateProverLocal.checksum(),
            );
            expect(feePrivateVerifier.checksum()).toEqual(
                feePrivateVerifierLocal.checksum(),
            );
            expect(feePublicProver.checksum()).toEqual(
                feePublicProverLocal.checksum(),
            );
            expect(feePublicVerifier.checksum()).toEqual(
                feePublicVerifierLocal.checksum(),
            );
            expect(joinProver.checksum()).toEqual(joinProverLocal.checksum());
            expect(joinVerifier.checksum()).toEqual(
                joinVerifierLocal.checksum(),
            );
            expect(splitProver.checksum()).toEqual(splitProverLocal.checksum());
            expect(splitVerifier.checksum()).toEqual(
                splitVerifierLocal.checksum(),
            );
            expect(transferPrivateProver.checksum()).toEqual(
                transferPrivateProverLocal.checksum(),
            );
            expect(transferPrivateVerifier.checksum()).toEqual(
                transferPrivateVerifierLocal.checksum(),
            );
            expect(transferPrivateToPublicProver.checksum()).toEqual(
                transferPrivateToPublicProverLocal.checksum(),
            );
            expect(transferPrivateToPublicVerifier.checksum()).toEqual(
                transferPrivateToPublicVerifierLocal.checksum(),
            );
            expect(transferPublicProver.checksum()).toEqual(
                transferPublicProverLocal.checksum(),
            );
            expect(transferPublicVerifier.checksum()).toEqual(
                transferPublicVerifierLocal.checksum(),
            );
            expect(transferPublicToPrivateProver.checksum()).toEqual(
                transferPublicToPrivateProverLocal.checksum(),
            );
            expect(transferPublicToPrivateVerifier.checksum()).toEqual(
                transferPublicToPrivateVerifierLocal.checksum(),
            );
            expect(unbondPublicProver.checksum()).toEqual(
                unbondPublicProverLocal.checksum(),
            );
            expect(unbondPublicVerifier.checksum()).toEqual(
            ...
            expect(bondPublicVerifierLocal.isBondPublicVerifier()).toEqual(
                true,
            );
            expect(
            ...
            expect(feePrivateVerifierLocal.isFeePrivateVerifier()).toEqual(
                true,
            );
            ...
            expect(transferPublicProverLocal.isTransferPublicProver()).toEqual(
                true,
            );
            expect(
            ...
            expect(unbondPublicProverLocal.isUnbondPublicProver()).toEqual(
                true,
            );
            expect(unbondPublicVerifierLocal.isUnbondPublicVerifier()).toEqual(
                true,
            );
        }, 380000);
    });
