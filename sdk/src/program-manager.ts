...
/**
 * Options for creating a ProvingRequest from a pre-signed ExecutionRequest.
 *
 * @property {ExecutionRequest} request - A pre-signed execution request to build the ProvingRequest from.
 * @property {boolean} [broadcast] - Whether to broadcast the Transaction generated by the remote prover to the Aleo network.
 */
interface ProvingRequestFromRequestOptions {
...
class ProgramManager {
    ...
    async provingRequest(
        options: ProvingRequestOptions | ProvingRequestFromRequestOptions,
    ): Promise<ProvingRequest> {
        // If a pre-signed request is provided, create the ProvingRequest directly from it.
        if ('request' in options) {
            const { request, broadcast = false } = options;
            const authorization = Authorization.new(request);
            return ProvingRequest.new(authorization, undefined, broadcast);
        }
...
export { ProgramManager, AuthorizationOptions, FeeAuthorizationOptions, ExecuteOptions, ProvingRequestOptions, ProvingRequestFromRequestOptions };
