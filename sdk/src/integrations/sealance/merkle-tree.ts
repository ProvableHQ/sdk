...
class SealanceMerkleTree {
    ...
    buildTree(leaves: string[]): bigint[] {
        ...
        while (levelSize > 1) {
            ...
            for (let i = 0; i < levelSize; i += 2) {
                ...
                const prefix =
                    leaves.length === levelSize ? "1field" : "0field";
    ...
    generateLeaves(addresses: string[], maxTreeDepth: number = 15): string[] {
        ...
        if (addresses.length > maxNumLeaves) {
            throw new Error(
                `Leaves limit exceeded. Max: ${maxNumLeaves}, provided: ${addresses.length}`,
            );
        }
        ...
        const sortedFieldElements = sortedFields.map(
            (field) => field.toString() + "field",
        );

        // Pad with zeros to reach power of 2
        const fullTree = Array(
            Math.max(numLeaves - sortedFieldElements.length, 0),
        ).fill("0field");
    ...
    getLeafIndices(merkleTree: bigint[], address: string): [number, number] {
        ...
        let rightLeafIndex = leaves.findIndex(
            (leaf: bigint) => addressBigInt <= leaf,
        );
     ...
     */
    formatMerkleProof(
        proof: { siblings: bigint[]; leaf_index: number }[],
    ): string {
