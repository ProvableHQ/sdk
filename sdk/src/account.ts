...
export class Account {
    ...
    private assertNotDestroyed(): void {
        if (this._destroyed) {
            throw new Error(
                "Account has been destroyed. Create a new Account instance.",
            );
        }
    }
    ...
    generateRecordViewKey(recordCiphertext: RecordCiphertext | string): Field {
        ...
        if (!recordCiphertext.isOwner(this._viewKey)) {
            throw new Error(
                "The record ciphertext does not belong to this account",
            );
        }
        return EncryptionToolkit.generateRecordViewKey(
            this._viewKey,
            recordCiphertext,
        );
    }
