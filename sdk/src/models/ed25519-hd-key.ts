// https://github.com/MystenLabs/sui/blob/main/sdk/typescript/src/utils/ed25519-hd-key.ts
// Apache-2.0
...
export const getMasterKeyFromSeed = (seed: Uint8Array): Keys => {
	const h = hmac.create(sha512, ED25519_CURVE);
	const I = h.update(seed).digest();
...
export const derivePath = (path: string, seed: Uint8Array, offset = HARDENED_OFFSET): Keys => {
	if (!isValidPath(path)) {
...
export function isValidHardenedPath(path: string): boolean {
	if (!new RegExp("^m\\/44'\\/41440'\\/[0-9]+'\\/[0-9]+'\\/[0-9]+'+$").test(path)) {
		return false;
	}
	return true;
};
