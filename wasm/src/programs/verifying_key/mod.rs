...
mod tests {
    ...
    use crate::types;
    use crate::types::native::BASE_URL;
    ...
    async fn test_metadata() {
        crate::verifying_key::metadata::Metadata;

        let base_url: &str = types::native::BASE_URL;
        let fee_public_metadata = Metadata::fee_public();
        let fee_public_verifier = VerifyingKey::from_string(&fee_public_metadata.verifying_key).unwrap();
        let fee_public_prover_uri = base_url.to_string() + fee_public_metadata.metadata.
    }
}
