...
class HybridAddressTestnet : public HybridAddressSpec {
 public:
  explicit HybridAddressTestnet();
  
  // Destructor - clean up Rust resources if needed
  ~HybridAddressTestnet();
  ...
  // Static factory method to create HybridAddressTestnet from string
  static std::shared_ptr<HybridAddressTestnet> fromString(const std::string& addressString);

  // Internal constructor for creating from address handle (public for make_shared)
  explicit HybridAddressTestnet(uint64_t addressHandle);
