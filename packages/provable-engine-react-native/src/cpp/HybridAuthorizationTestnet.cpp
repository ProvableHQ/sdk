...
HybridAuthorizationTestnet::HybridAuthorizationTestnet(uint64_t authorizationHandle) : HybridObject([]() -> const char* { static std::string k = std::string(TAG) + "Testnet"; return k.c_str(); }()), _authorizationHandle(authorizationHandle) {}

// Destructor - clean up Rust resources
HybridAuthorizationTestnet::~HybridAuthorizationTestnet() {
  if (_authorizationHandle != 0) {
    shield_testnet::destroy_authorization(_authorizationHandle);
  }
}

// Factory methods
std::shared_ptr<HybridAuthorizationSpec> HybridAuthorizationTestnet::fromString(const std::string& authorizationString) {
  auto result = shield_testnet::authorization_from_string(rust::String(authorizationString));
  if (result.success) {
...
std::shared_ptr<HybridAuthorizationSpec> HybridAuthorizationTestnet::fromBytesLe(const std::shared_ptr<ArrayBuffer>& bytes) {
  rust::Vec<uint8_t> bytesVec;
  for (size_t i = 0; i < bytes->size(); ++i) {
    bytesVec.push_back(bytes->data()[i]);
  }

  auto result = shield_testnet::authorization_from_bytes_le(bytesVec);
  if (result.success) {
    return std::make_shared<HybridAuthorizationTestnet>(result.handle);
  } else {
    throw std::runtime_error(std::string(result.error));
...
std::shared_ptr<HybridAuthorizationSpec> HybridAuthorizationTestnet::fromRequestsAndTransitions(const std::vector<RequestJSON>& requests,
  ...
  auto result = shield_testnet::authorization_from_requests_and_transitions(std::move(requestsVec), std::move(transitionsVec));
  if (result.success) {
    return std::make_shared<HybridAuthorizationTestnet>(result.handle);
  } else {
    throw std::runtime_error(std::string(result.error));
...
std::string HybridAuthorizationTestnet::toString() {
  if (_authorizationHandle == 0) {
    throw std::runtime_error("Authorization handle is null");
  }

    auto result = shield_testnet::authorization_to_string(_authorizationHandle);
  if (result.success) {
...
std::shared_ptr<ArrayBuffer> HybridAuthorizationTestnet::toBytesLe() {
  if (_authorizationHandle == 0) {
    throw std::runtime_error("Authorization handle is null");
  }

    auto result = shield_testnet::authorization_to_bytes_le(_authorizationHandle);
  if (result.success) {
...
AuthorizationJSON HybridAuthorizationTestnet::toJSON() {
  if (_authorizationHandle == 0) {
    throw std::runtime_error("Authorization handle is null");
  }
...
// Accessors
std::vector<RequestJSON> HybridAuthorizationTestnet::getRequests() {
  if (_authorizationHandle == 0) {
    throw std::runtime_error("Authorization handle is null");
  }

  auto result = shield_testnet::authorization_get_requests(_authorizationHandle);
  if (!result.success) {
    throw std::runtime_error(std::string(result.error));
  }
...
std::vector<TransitionJSON> HybridAuthorizationTestnet::getTransitions() {
  if (_authorizationHandle == 0) {
    throw std::runtime_error("Authorization handle is null");
  }

  auto result = shield_testnet::authorization_get_transitions(_authorizationHandle);
  if (!result.success) {
    throw std::runtime_error(std::string(result.error));
  }
...
// Utility methods
std::shared_ptr<HybridAuthorizationSpec> HybridAuthorizationTestnet::replicate() {
  if (_authorizationHandle == 0) {
    throw std::runtime_error("Authorization handle is null");
  }

  auto result = shield_testnet::authorization_replicate(_authorizationHandle);
  if (result.success) {
    return std::make_shared<HybridAuthorizationTestnet>(result.handle);
  } else {
    throw std::runtime_error(std::string(result.error));
...
bool HybridAuthorizationTestnet::verify() {
  if (_authorizationHandle == 0) {
    return false;
  }

  return shield_testnet::authorization_verify(_authorizationHandle);
}

// New methods for comprehensive API coverage
bool HybridAuthorizationTestnet::isEqual(const std::shared_ptr<HybridAuthorizationSpec>& other) {
  if (_authorizationHandle == 0) {
    return false;
  }

  auto otherAuthorization = std::static_pointer_cast<HybridAuthorizationTestnet>(other);
  if (otherAuthorization->_authorizationHandle == 0) {
    return false;
  }

  return shield_testnet::authorization_equals(_authorizationHandle, otherAuthorization->_authorizationHandle);
}

double HybridAuthorizationTestnet::len() {
  if (_authorizationHandle == 0) {
    return 0;
  }

  return shield_testnet::authorization_len(_authorizationHandle);
}

bool HybridAuthorizationTestnet::isEmpty() {
  if (_authorizationHandle == 0) {
    return true;
  }

  return shield_testnet::authorization_is_empty(_authorizationHandle);
}

bool HybridAuthorizationTestnet::isFeePrivate() {
  if (_authorizationHandle == 0) {
    return false;
  }

  return shield_testnet::authorization_is_fee_private(_authorizationHandle);
}

bool HybridAuthorizationTestnet::isFeePublic() {
  if (_authorizationHandle == 0) {
    return false;
  }

  return shield_testnet::authorization_is_fee_public(_authorizationHandle);
}

bool HybridAuthorizationTestnet::isSplit() {
  if (_authorizationHandle == 0) {
    return false;
  }

  return shield_testnet::authorization_is_split(_authorizationHandle);
}

void HybridAuthorizationTestnet::insertTransition(const Transition& transition) {
  if (_authorizationHandle == 0) {
    throw std::runtime_error("Authorization handle is null");
  }
  ...
  auto result = shield_testnet::authorization_insert_transition(
...
std::shared_ptr<HybridFieldSpec> HybridAuthorizationTestnet::toExecutionId() {
  if (_authorizationHandle == 0) {
    throw std::runtime_error("Authorization handle is null");
  }

  auto result = shield_testnet::authorization_to_execution_id(_authorizationHandle);
  if (result.success) {
...
std::vector<Transition> HybridAuthorizationTestnet::transitions() {
  if (_authorizationHandle == 0) {
    throw std::runtime_error("Authorization handle is null");
  }
