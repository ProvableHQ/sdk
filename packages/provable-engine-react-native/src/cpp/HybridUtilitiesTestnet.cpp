...
void unwrapBoolResult(shield_testnet::BoolResult result) {
    if (!result.success) {
        throw std::runtime_error(std::string(result.error));
    }
}
...
ParsedRecordCiphertext HybridUtilitiesTestnet::parseRecordCiphertext(const std::string& recordPlaintext) {
    try {
...
std::string HybridUtilitiesTestnet::parseNativeAmount(const std::string& plaintext) {
    try {
...
std::string HybridUtilitiesTestnet::parseTokenAmount(
...
std::string HybridUtilitiesTestnet::getTokenIdFromRecordPlaintext(
    const std::string& plaintext,
    const std::string& programId
) {
    try {
...
std::vector<std::string> HybridUtilitiesTestnet::constructTransactionInputs(
...
std::vector<std::string> HybridUtilitiesTestnet::constructSplitTxInputs(
    const std::string& programId,
    const std::string& recordPlaintext,
    const std::string& amount
) {
...
std::string HybridUtilitiesTestnet::getHashedString(const std::string& idPlaintextBits) {
    try {
        ...
        auto plaintextHandle = shield_testnet::plaintext_from_string(idPlaintextBits);
        ...
        auto hashResult = shield_testnet::bhp256_hash(std::move(bitsResult.bytes));
...
std::string HybridUtilitiesTestnet::getMappingNameForProgram(const std::string& programId) {
    try {
        if (!isValidProgramId(programId)) {
            throw std::invalid_argument("Invalid program ID");
        }
...
std::string HybridUtilitiesTestnet::parseU128(const std::string& balance) {
    try {
        if (balance.empty()) {
            return "0";
        }
...
std::string HybridUtilitiesTestnet::parseU64(const std::string& balance) {
    try {
        if (balance.empty()) {
            return "0";
        }
...
TokenRegistryBalance HybridUtilitiesTestnet::parseTokenRegistryBalance(const std::string& res) {
    try {
...
bool HybridUtilitiesTestnet::verifyMnemonic(const std::string& mnemonic) {
    try {
        if (mnemonic.empty()) {
            return false;
        }
...
std::string HybridUtilitiesTestnet::cryptoBoxSealBase64(
    ...
    try {
        ...
        auto result = shield_testnet::crypto_box_seal_base64(
...
std::shared_ptr<ArrayBuffer> HybridUtilitiesTestnet::cryptoBoxSealOpenBase64(
    const std::string& publicKeyB64,
    const std::string& privateKeyB64,
    const std::string& sealedB64
) {
    try {
        auto result = shield_testnet::crypto_box_seal_open_base64(
            rust::String(publicKeyB64),
...
std::string HybridUtilitiesTestnet::extractFieldFromPlaintext(const std::string& plaintext, const std::string& fieldName) const {
    try {
...
bool HybridUtilitiesTestnet::isValidAleoAddress(const std::string& address) const {
    try {
...
bool HybridUtilitiesTestnet::isValidProgramId(const std::string& programId) const {
    try {
...
bool HybridUtilitiesTestnet::isValidTokenId(const std::string& tokenId) const {
    try {
...
std::string HybridUtilitiesTestnet::formatAmount(const std::string& amount, int decimals) const {
    try {
...
std::vector<std::string> HybridUtilitiesTestnet::splitString(const std::string& str, char delimiter) const {
...
std::string HybridUtilitiesTestnet::trimWhitespace(const std::string& str) const {
