...
class HybridProgramTestnet final : public HybridProgramSpec {
 public:
  HybridProgramTestnet();
  explicit HybridProgramTestnet(rust::Box<shield_testnet::ProgramHandle>&& program);
  ...
  rust::Box<shield_testnet::ProgramHandle>& program();
  const rust::Box<shield_testnet::ProgramHandle>& program() const;

 private:
  shield_testnet::ProgramHandle& ensureProgram() const;
  std::vector<std::string> convertVec(const rust::Vec<rust::String>& vec) const;

 private:
  mutable std::shared_ptr<rust::Box<shield_testnet::ProgramHandle>> _program;
};
