...
import { FIVE_MINUTES, RECORD_DOMAIN } from "./constants";
...
import type { EncryptedRegistrationRequest } from "./models/record-scanner/encryptedRegistrationRequest";
import type { EncryptedRecordsResult } from "./models/record-scanner/encryterRecordsResult";
import {
  DecryptionNotEnabledError,
  RecordNotFoundError,
  type RecordScannerFailure,
  RecordScannerRequestError,
  UUIDError,
  ViewKeyNotStoredError,
} from "./models/record-scanner/error";
import type { OwnedFilter } from "./models/record-scanner/ownedFilter";
import type { OwnedRecordsResult } from "./models/record-scanner/ownedRecordsResult";
import type { RecordsFilter } from "./models/record-scanner/recordsFilter";
import type { RegistrationRequest } from "./models/record-scanner/registrationRequest";
import type { RegisterResult } from "./models/record-scanner/registrationResult";
import type { RevokeResult } from "./models/record-scanner/revokeResult";
import type { SerialNumbersResult } from "./models/record-scanner/serialNumberResult";
import type { StatusResult } from "./models/record-scanner/statusResult";
import type { TagsResult } from "./models/record-scanner/tagsResult";
import type { RecordProvider } from "./network-record-provider";
import { encryptRegistrationRequest } from "./security";
import { parseJSON, post } from "./utils";
import { ViewKey } from "./view-key";
import { RecordCiphertext, RecordPlaintext } from "./wasm";

export interface RecordScannerJWTData extends JWTData {}
...
export class RecordScanner implements RecordProvider {
  ...
  constructor(options: RecordScannerOptions) {
    ...
    if (this.account) {
      ...
      this.setUuid(this.account.viewKeyObject());
      // Add the view key to the scanner's view keys.
      this.addViewKey(this.account.viewKeyObject());
    }
  ...
  setApiKey(apiKey: string | { header: string; value: string }) {
    this.apiKey =
      typeof apiKey === "string" ? { header: "X-Provable-API-Key", value: apiKey } : apiKey;
  }
  ...
  private getViewKeyForUuid(uuid: string): ViewKey | undefined {
    ...
    const accountVk = this.account?.viewKeyObject();
  ...
  setAccount(account: Account) {
    const existingVk = this.account?.viewKeyObject();
    // Set the account on the scanner.
    this.account = account;
    // Set the uuid on the scanner using the view key from the account.
    this.setUuid(account.viewKeyObject());
    // Add the view key to the scanner's view keys.
    this.addViewKey(account.viewKeyObject());
   ...
   * @returns {Promise<RegisterResult>} `{ ok: true, data }` on success, or `{ ok: false, status, error }` on failure.
   */
  async registerEncrypted(viewKey: ViewKey, startBlock: number): Promise<RegisterResult> {
    try {
  ...
  private clearLocalStateForUuid(uuidStr: string): void {
    ...
    if (
      this.account != null &&
      this.computeUUID(this.account.viewKeyObject()).toString() === uuidStr
    ) {
   ...
   *
   * @param {Int64} microcredits The amount of microcredits to find.
   * @param {OwnedFilter} searchParameters The filter to use to find the record.
   * @returns {Promise<OwnedRecord>} The record.
   */
    ...
    try {
      ...
      const record = records.find((r) => {
        ...
        try {
          ...
          const amount = parseInt(amountStr.replace("u64", ""), 10);
      ...
      if (!record) {
        throw new RecordNotFoundError(
          `No records found matching the supplied search filter:\n${JSON.stringify(
            ...
            null,
            2
          )}. Decryption of owned records MUST be enabled and the ViewKey matching the UUID must be stored in the RecordScanner object to perform decryption.`,
          searchParameters
        );
      }
   ...
   *
   * @param {Int64[]} microcreditAmounts The amounts of microcredits to find.
   * @param {OwnedFilter} searchParameters The filter to use to find the records.
   * @returns {Promise<OwnedRecord[]>} The records
   */
    ...
    try {
      ...
      return records.filter((r) => {
        ...
        try {
          ...
          return microcreditAmounts.includes(BigInt(amount.replace("u64", "")));
        } catch {
          return false;
        }
      });
