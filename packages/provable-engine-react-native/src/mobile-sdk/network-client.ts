...
import { applyNetworkToHost, getNetwork, type NetworkName, setNetwork } from "./current-network";
import type { BlockJSON } from "./models/blockJSON";
import type { ConfirmedTransactionJSON } from "./models/confirmed_transaction";
import type { CryptoBoxPubKey } from "./models/cryptoBoxPubkey";
import type { EncryptedProvingRequest } from "./models/encryptedProvingRequest";
import type { JWTData } from "./models/jwtData";
import {
  isProveApiErrorBody,
  isProvingResponse,
  type ProveApiErrorBody,
  type ProvingRequestError,
  type ProvingResponse,
  type ProvingResult,
} from "./models/provingResponse";
import type { TransactionJSON } from "./models/transaction/transactionJSON";
import { encryptProvingRequest } from "./security";
import { environment, get, logAndThrow, parseJSON, post, retryWithBackoff } from "./utils";
import {
  Address,
  Plaintext,
  PrivateKey,
...
export type ProgramImports = { [key: string]: string | Program };

export interface AleoNetworkClientOptions {
 ...
 * @property dpsPrivacy {boolean} Whether to use DPS privacy mode for the proving request.
 */
export interface DelegatedProvingParams {
...
export class AleoNetworkClient {
  ...
  get host(): string {
    return applyNetworkToHost(this.baseHost);
  }
  headers: { [key: string]: string };
  ...
  constructor(host: string, options?: AleoNetworkClientOptions) {
    const sanitized = host.replace(/\/+$/, "").replace(/\/(mainnet|testnet)$/, "");
    this.baseHost = sanitized;
    this.ctx = {};
    ...
    if (options) {
      ...
      if (options.proverUri) {
        ...
        this.proverUri = `${options.proverUri}/${this.network}`;
      }

      if (options.recordScannerUri) {
        // If a record scanner uri was specified, set the record scanner uri.
        this.recordScannerUri = `${options.recordScannerUri}/${this.network}`;
      }
    } else {
      this.headers = {
        // This is replaced by the actual version by a Rollup plugin
  ...
  setProverUri(proverUri: string) {
    this.proverUri = `${proverUri}/${this.network}`;
  }
  ...
  setRecordScannerUri(recordScannerUri: string) {
    this.recordScannerUri = `${recordScannerUri}/${this.network}`;
  }
   ...
   * @param {Int64[]} amounts - The amounts (in microcredits) to search for (eg. [100, 200, 3000])
   * @param {Int64} maxMicrocredits - The maximum number of microcredits to search for
  ...
  async findRecords(
    ...
    programs?: string[],
    amounts?: Int64[] | undefined,
    maxMicrocredits?: Int64 | undefined,
    ...
    if (typeof privateKey === "undefined") {
      if (typeof this.account === "undefined") {
        ...
        resolvedPrivateKey = new PrivateKey(this.account.privateKey());
      }
    } else {
      try {
        resolvedPrivateKey =
    ...
    const viewKey = resolvedPrivateKey.toViewKey();
            ...
            for (let j = 0; j < transactions.length; j++) {
              ...
              if (confirmedTransaction?.type === "execute") {
                ...
                if (
                  ...
                  !(typeof transaction.execution.transitions === "undefined")
                ) {
                  for (let k = 0; k < transaction.execution.transitions.length; k++) {
                    ...
                    if (!(typeof programs === "undefined") && transition?.program) {
                            ...
                            if (record.isOwner(viewKey)) {
                              ...
                              if (unspent) {
                                const recordViewKeyField = recordPlaintext
                                  .recordViewKey(viewKey)
                                  .toString();
                                let serialNumber: string | undefined;

                                try {
                                  serialNumber = recordPlaintext.serialNumberString(
                                    resolvedPrivateKey,
                                    "credits.aleo",
                                    "credits",
                                    recordViewKeyField
                                  ...
                                  console.warn(
                                    "serialNumber derivation is not yet supported on mobile; skipping spent check",
                                    error
                                  );
                                }
                                ...
                                if (serialNumber) {
                                  try {
                                    await retryWithBackoff(() =>
                                      this.getTransitionId(serialNumber)
                                    );
                                    continue;
                                  } catch (_error) {
                              ...
                              if (!amounts) {
                                ...
                                if (typeof maxMicrocredits === "bigint") {
                              ...
                              if (!(typeof amounts === "undefined") && amounts.length > 0) {
                                if (
                                  recordPlaintext.microcredits() >= (amounts?.[records.length] ?? 0)
                                ) {
                                  records.push(recordPlaintext);
                                  // If the user specified a maximum number of microcredits, check if the search has found enough
                                  if (typeof maxMicrocredits === "bigint") {
  ...
  async getProgram(programId: string, edition?: number): Promise<string> {
    try {
      ...
      if (typeof edition === "number") {
        return await this.fetchData<string>(`/program/${programId}/${edition}`);
      } else {
  ...
  async getProgramObject(inputProgram: string, edition?: number): Promise<Program> {
    try {
      ...
      return Program.fromString(<string>await this.getProgram(inputProgram, edition));
    } catch (error) {
  ...
  async getProgramImports(
    ...
    try {
      ...
      for (let i = 0; i < importList.length; i++) {
        ...
        if (import_id !== undefined && !Object.hasOwn(imports, import_id)) {
  ...
  /**
   * Get the state path for a given commitment.
   *
   * @param {string} commitment - The commitment field string (e.g., "1234567890field")
   * @returns {Promise<string>} - The state path as a JSON string
   * @example
   * const statePath = await networkClient.getStatePath("1234567890field");
   */
  async getStatePath(commitment: string): Promise<string> {
    try {
      this.ctx = { "X-ALEO-METHOD": "getStatePath" };
      return await this.fetchData<string>(`/statePath/${commitment}`);
    } catch (error) {
      throw new Error(`Error fetching state path for commitment ${commitment}: ${error}`);
    } finally {
      this.ctx = {};
    }
  }
  ...
  async submitTransaction(transaction: Transaction | string): Promise<string> {
    const transactionString =
      transaction instanceof Transaction ? transaction.toString() : transaction;
    try {
      ...
      const response = await retryWithBackoff(() =>
        this._sendPost(`${this.host}/${endpoint}`, {
          ...
          headers: Object.assign(
            {},
            { ...this.headers, "X-ALEO-METHOD": "submitTransaction" },
            {
  ...
  private async refreshJwt(apiKey: string, consumerId: string): Promise<JWTData> {
    ...
    const response = await post(`${this.host}/jwts/${consumerId}`, {
      headers: {
   ...
   * @returns {Promise<ProvingResponse>} The ProvingResponse containing the transaction result and the result of the broadcast if the `broadcast` flag was set to `true`.
   */
  // async submitProvingRequestOLD(options: DelegatedProvingParams): Promise<ProvingResponse> {
    ...
    return new Promise((resolve, reject) => {
      const interval = setInterval(async () => {
        ...
        try {
          ...
          const data = parseJSON(await res.text()) as ConfirmedTransactionJSON;
