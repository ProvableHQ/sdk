    ...
    if (ev.data.type === "ALEO_EXECUTE_PROGRAM_LOCAL") {
        ...
        (async function () {
            try {
                ...
                if (!program.hasFunction(aleoFunction)) {
                    throw new Error(
                        `Program ${program_id} does not contain function ${aleoFunction}`,
                    );
                }
                ...
                if (execution) {
                    ...
                    console.log(
                        "Execution verified successfully: " + execution,
                    );
                } else {
                    execution = "";
                }
        ...
        (async function () {
            try {
                ...
                // Ensure the program is valid and that it contains the function specified
                const program =
                    await programManager.createProgramFromSource(remoteProgram);
                const program_id = program.id();
                if (!program.hasFunction(aleoFunction)) {
                    throw new Error(
                        `Program ${program_id} does not contain function ${aleoFunction}`,
        ...
        (async function () {
            try {
                ...
                // Ensure the program is valid and that it contains the function specified
                const program =
                    await programManager.networkClient.getProgramObject(
                        remoteProgram,
                    );
                const program_id = program.id();
                ...
                const cacheKey = `${program_id}:${aleoFunction}`;
                const imports =
                    await programManager.networkClient.getProgramImports(
                        remoteProgram,
                    );
                ...
                try {
                    edition =
                        programManager.networkClient.getLatestProgramEdition(
                            program_id,
                        );
                } catch {
                    console.warn(
                        `Error finding edition for ${program_id}. Assuming edition 1.`,
                    );
                }

                // Estimate the execution fee
                let executeFee =
                    await aleo.ProgramManagerBase.estimateExecutionFee(
                        remoteProgram,
                        aleoFunction,
                        imports,
                        edition,
                ...
                console.log(
                    `Execution Fee Estimation: ${executeFee} microcredits`,
                );
                self.postMessage({
        ...
        (async function () {
            try {
                ...
                console.log(
                    `Deployment Fee Estimation: ${deploymentFee} microcredits`,
                );
                self.postMessage({
        ...
        const { program, privateKey, fee, privateFee, feeRecord, url } =
            ev.data;
        ...
        (async function () {
            try {
                ...
                try {
                    ...
                    throw new Error(
                        `Invalid program, ensure the program is valid and try again.`,
                    );
                }
                ...
                try {
                    await programManager.networkClient.getProgram(
                        programObject.id(),
                    );
                    programExists = true;
                } catch (e) {
                ...
                if (programExists) {
                    throw new Error(
                        `Program ${programObject.id()} already exists on the network`,
                    );
                }
                ...
                await programManager.networkClient.submitTransaction(
                    transaction,
                );
                ...
                // Return the transaction id to the main thread
                console.log(
                    `Web worker: Deployment transaction ${transaction_id} created in ${performance.now() - startTime} ms`,
                );
                self.postMessage({
