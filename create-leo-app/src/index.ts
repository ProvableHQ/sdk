...
async function init() {
    ...
    if (argTemplate && TEMPLATES.includes(argTemplate)) {
        for (const framework of FRAMEWORKS) {
            const match = framework.variants.find(
                (v) => v.name === argTemplate,
            );
            if (match) {
    ...
    if (!selectedVariantObj && framework) {
        selectedVariantObj = framework.variants.find(
            (v: FrameworkVariant) => v.name === variant,
        );
    }
    ...
    switch (pkgManager) {
        default:
            ...
            console.log(
                `  ${pkgManager} ${runCommand === "dev" ? "run dev" : runCommand}`,
            );
            break;
    }
    console.log();
