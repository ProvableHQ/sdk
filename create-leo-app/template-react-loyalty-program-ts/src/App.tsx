...
function App() {
    ...
    const [selectedRewardType, setSelectedRewardType] = useState<number>(
        RewardType.Discount,
    );
    const [pointsCost, setPointsCost] = useState<number>(100);

    // Config state (for conditional UI)
    const [provingMode, setProvingMode] = useState<"local" | "delegated">(
        "local",
    );
    ...
    const handleMintCard = async () => {
        ...
        setLoading(
            "Minting loyalty card (this may take a moment due to hash operations)...",
        );
        try {
            const aleoWorker = await getAleoWorker();
            ...
            setStatus(
                `Loyalty card minted! You have ${newCard.points} points (${TIER_NAMES[newCard.tier]} tier)`,
            );
        } catch (error) {
    ...
    const updateSelectedCard = (updatedCard: LoyaltyCard) => {
        setCards((prev) =>
            prev.map((c, i) => (i === selectedCardIndex ? updatedCard : c)),
        );
    };
    ...
    const handleAddPoints = async () => {
        ...
        setLoading(
            `Adding ${pointsToAdd} points (computing hash for audit trail)...`,
        );
        try {
            const aleoWorker = await getAleoWorker();
            // Clean API: pass the card object directly
            const updatedCard = await aleoWorker.addPoints(
                selectedCard,
                pointsToAdd,
            );
    ...
    const handleSplitCard = async () => {
        ...
        if (pointsToKeep >= selectedCard.points) {
            setStatus(
                `Points to keep must be less than card balance (${selectedCard.points})`,
            );
            return;
        }
        ...
        setLoading(
            `Splitting card (keep ${pointsToKeep}, split off ${splitPoints})...`,
        );
        try {
            const aleoWorker = await getAleoWorker();
            const result: SplitResult = await aleoWorker.splitCardV2(
                selectedCard,
                pointsToKeep,
            );
    ...
    const handleRedeem = async () => {
        ...
        if (selectedCard.points < pointsCost) {
            setStatus(
                `Not enough points! You have ${selectedCard.points}, need ${pointsCost}`,
            );
            return;
        }
    ...
    const handleUseVoucher = async (voucherToUse: RewardVoucher) => {
        ...
        try {
            ...
            setVouchers((prev) =>
                prev.filter((v) => v.raw !== voucherToUse.raw),
            );
    ...
    if (!initialized) {
        return <div className="loading">Initializing loyalty program...</div>;
    }
                            ...
                            <div className="card-selector">
                                {cards.map((c, i) => (
                                    <button
                                        ...
                                        disabled={!!loading}
                                    >
                                        <span className="card-tab-points">
                                            {c.points}
                                        </span>
                                        <span
                                            className="card-tab-tier"
                                            style={{
                ...
                {selectedCard && (
                    <div className="redeem-controls">
                        <div className="input-group">
                            ...
                            >
                                <option value={RewardType.Discount}>
                                    Discount
                                </option>
                                <option value={RewardType.Freebie}>
                                    Freebie
                                </option>
                                <option value={RewardType.Upgrade}>
                                    Upgrade
                                </option>
                            </select>
                        </div>
                        ...
                        <button
                            ...
                            disabled={
                                !!loading || selectedCard.points < pointsCost
                            }
                        >
                            Redeem for Voucher
                        </button>
...
await loyalty.useVoucher(voucher);`}
                ...
                <ul>
                    <li>
                        <strong>Multi-Program:</strong> Uses loyalty_token.aleo
                        and loyalty_rewards.aleo
                    </li>
                    <li>
                        <strong>Hash Functions:</strong> BHP256 generates unique
                        card/voucher IDs
                    </li>
                    <li>
                        <strong>Typed Records:</strong> LoyaltyCard and
                        RewardVoucher interfaces
                    </li>
                    <li>
                        <strong>Enums:</strong> RewardType.Discount,
                        CardTier.Gold, etc.
                    </li>
                </ul>
            </div>
