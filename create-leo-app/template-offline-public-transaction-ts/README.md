...
and `VerifyingKey` in order to build a zero-knowledge zk-SNARK proof of
...
`bond_public` `unbond_public` `claim_unbond_public`
...
downloaded to the `dist/keys` folder. The machine can then be disconnected from
the internet and the `OfflineKeyProvider` will search this directory for the
...
Once the keys are downloaded to your local machine, the offline transactions
will be built without requiring an internet connection.
