  ...
  if (ev.data.type === "ALEO_EXECUTE_PROGRAM_LOCAL") {
    ...
    (async function () {
      try {
        ...
        // Ensure the program is valid and that it contains the function specified
        const program = await programManager.networkClient.getProgramObject(
          remoteProgram,
        );
        const program_id = program.id();
        if (!program.hasFunction(aleoFunction)) {
    ...
    const { remoteProgram, aleoFunction, inputs, url } = ev.data;
    ...
    (async function () {
      try {
        ...
        let executeFee =
          await programManager.executionEngine.estimateExecutionFee(
            new aleo.PrivateKey(),
            remoteProgram,
            aleoFunction,
            inputs,
    ...
    const { recordOne, recordTwo, fee, feeRecord, privateKey, url } = ev.data;
